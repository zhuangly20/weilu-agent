...
PHASE_TURN_BUDGETS = {1: 2, 2: 2, 3: 3, 4: 3, 5: 2, 6: 3, 7: 2, 8: 2}
...
def build_system_prompt(state: GroupState, action: str) -> str:
    ...
    action_text = {
        ...
        "discuss": (
            ...
            "每位团友最多出现一次，不要轮流评价用户，也不要把每个话题折回用户。"
            "最后由一位团友给真人一个清楚的接话口：只问一个具体问题，并说明也可以继续听；"
    ...
    if state.phase == 1 and action == "discuss":
        action_text += (
            "\n这是开场自我介绍后的简短团体互动。先让恰好2位团友直接回应用户或彼此。"
            "整轮只能有一位团友向用户提出一个问题；其他团友必须分享自己的具体经历或回应团友，"
            "不得每人都问用户，不得比较谁的阶段更高、压力更重。"
            "随后小晴只用1—2句提炼互动中的共同性、差异和已经显现的力量，赋能必须有刚才的行为证据，"
            "不能只是夸奖，也不能预告‘我会回来问你’或对用户做一对一访谈。"
            "小晴把话筒明确交给一位最相关的团友；由这位团友基于自己的真实好奇，"
            "向用户问一个具体、低负担、与双方经历都有关的问题。最后一句必须来自团友，"
            "让用户是在回应同伴而不是回应主持人。问题后要自然补一句：可以慢慢想，也可以继续听或停一下。"
            "不要笼统问‘想聊什么’，不要让三位团友连续围着用户提问。"
            "为给交棒留出篇幅，前面每位团友最多2句；本轮总计260—400字。"
        )
...
- 根据年级判断相对资历：低年级团友面对明显更高年级的用户，要体现校园中自然的尊重。只有用户明确偏好或性别称谓后才用“名字+学姐/学长”；不明确时可自然确认一次，不能擅自猜性别，也不能把高年级用户架成权威。
