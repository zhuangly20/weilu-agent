...
【小晴】晚上好呀，同学～我是**小晴**，小清心的AI团体助手化身，也是这间深夜小屋的主人。这里有炉火、有热茶，还有几位很有意思的炉友——没有评判，也没有建议轰炸。

【小晴】今晚有几场围炉活动，想参加哪一场？
1️⃣ **自我探索·围炉夜话**——"我是谁、我想要什么"
2️⃣ **学业压力·围炉夜话**——绩点、科研、同辈比较
3️⃣ **新生适应·围炉夜话**——想家、宿舍、新环境
5️⃣ **就业迷茫·围炉夜话**——工作、升学、Gap
🎨 **围炉画会**——任选一个主题，大家用文字轮流添笔，共同完成一幅真正的画（结束送给你）

每一场都会为你挑选几位AI炉友围坐陪伴；活动结束后，你还会收到一张根据当晚内容亲手写的明信片✨

回复数字或活动名就能报名；也可以直接跟我说说最近的困惑，我来帮你挑一场最合适的～

（小声说：围炉夜话是朋辈支持空间，不是心理治疗哦。如果你正处于难以承受的痛苦中，请一定联系学校心理中心或专业援助热线。）"""

MENU_RETRY_TEXT = """【小晴】（歪着头）炉边有点热闹，我没太听清你想参加哪一场——
1️⃣ 自我探索　2️⃣ 学业压力　3️⃣ 新生适应　5️⃣ 就业迷茫　🎨 围炉画会

回复数字或活动名就行；或者，直接跟我讲讲最近的心事也可以～"""

SEAT_REASK_TEXT = """【小晴】（歪头）没太听清——你是想换一位炉友，还是准备开炉啦？
想换谁就点名告诉我（比如"换掉苏轼"）；不换的话说声"开炉"，我们马上开始～"""

NO_MORE_SWAP_TEXT = """【小晴】（挠挠头）有点不好意思，剩下的炉友已经是我今晚能请到的全部啦……
要不我们先开炉试试？聊上两句要是真的不合拍，再回来找我商量～就这么定？"""
...
def build_invite_system_prompt(
    ...
    if form == "painting":
        scheme = (
            f"- 玩法：**围炉画会**（{theme_label}）。不用画笔——大家围炉坐定，"
        ...
        scheme = (
            f"- 玩法：**{theme_label}·围炉夜话**。大家围炉文字畅聊，"
    ...
    return "\n\n".join(
        [
            "你是\"围炉夜话\"的导演。本轮只有带领者小晴一个人发言，完成\"介绍与相邀\"。",
            "【空间设定】深夜、炉火、小木屋。氛围安全、松弛、不评判。",
            _leader_block(leader_cfg),
            "【今晚的炉友（小晴口头介绍，他们本人本轮不发言）】\n"
            + "\n".join(
            ...
            "不是治疗哦——不过大家的认真程度不打折。”\n"
            f"4) 介绍今晚请到的四位炉友（{intro_names}）：每人1-2句，"
            "姓名+身份背景+为什么今晚请TA，用小晴介绍朋友的口吻\n"
            "5) 结尾问：“这几位炉友合你眼缘吗？想换掉谁跟我说一声就行；"
            "不换的话，我们就开炉啦～”",
            "【输出格式（必须严格遵守）】\n"
...
def build_swap_system_prompt(
    ...
    task = (
        ...
        f"4) 新成员说一句入座招呼\n"
        "5) 小晴结尾再问：\"还有想换的吗？没有的话我们就开炉啦～\""
    )
    if note:
        task = note + "\n" + task
    return "\n\n".join(
        [
            "你是\"围炉夜话\"的导演。本轮完成一次炉友更换。",
            "【空间设定】深夜、炉火、小木屋。",
            *blocks,
            task,
            "【输出格式（必须严格遵守）】\n"
...
def _painting_stage_instruction(stage_id: str, theme: dict, team: list[dict]) -> str:
    ...
    if stage_id == "ignite":
        return head + (
            ...
            "3) 小晴预告：接下来四位炉友会依次添笔，然后轮到同学（一句带过，不提问）。"
        )
    if stage_id == "strokes":
        return head + (
...
def build_turn_system_prompt(
    ...
    return "\n\n".join(
        [
            "你是\"围炉夜话\"的导演。围炉夜话是一个深夜线上朋辈支持空间：一位真人同学"
            "和几位AI成员围炉夜谈。你的唯一任务：根据当前进度和真人同学的最后发言，"
            "写出本轮的团体对话（剧本式群聊）。",
            "【空间设定】深夜、炉火、小木屋。氛围安全、松弛、不评判，像老友夜谈。",
            "【你要扮演的角色】",
            _leader_block(leader_cfg),
...
def build_report_system_prompt(leader_cfg: dict, theme: dict) -> str:
    return "\n\n".join(
        [
            "你是\"围炉夜话\"的导演。本轮只有小晴一个人发言，生成上次围炉的《成长手记》。",
            _leader_block(leader_cfg),
            "【输出格式（必须严格遵守，逐行输出）】",
